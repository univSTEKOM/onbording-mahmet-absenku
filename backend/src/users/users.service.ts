...
import type { UpdateUserDto, AdminUpdateUserDto, UpdateUserStatusDto, AddNoteDto } from './users.schema'

type DrizzleDb = NodePgDatabase<typeof schema>

function stripInternal(userRow: Record<string, unknown>) {
  const { emailVerified, ...rest } = userRow
  return rest
}
...
export class UsersService {
  ...
  async getProfile(userId: string) {
    ...
    return stripInternal(row as unknown as Record<string, unknown>)
  }

  async updateProfile(userId: string, dto: UpdateUserDto, currentUserId: string) {
    if (userId !== currentUserId) throw new ForbiddenException('Anda hanya bisa mengubah profil sendiri')
    ...
    if (dto.faceDescriptor !== undefined) updateFields.faceDescriptor = dto.faceDescriptor
    ...
    const [updated] = await this.db.select().from(user).where(eq(user.id, userId)).limit(1)
    return stripInternal(updated as unknown as Record<string, unknown>)
  }
  ...
  async getPendingUsers() {
    ...
    return rows.map((r) => stripInternal(r as unknown as Record<string, unknown>))
  }

  async getAllUsers(params: { page?: number; limit?: number; q?: string; role?: string; status?: string }) {
    ...
    const [totalResult] = await this.db.select({ total: count() }).from(user).where(where)
    ...
    return {
      data: rows.map((r) => stripInternal(r as unknown as Record<string, unknown>)),
  ...
  async updateUserStatus(userId: string, dto: UpdateUserStatusDto) {
    ...
    if (dto.status === 'approved') {
      await this.db.update(user).set({ status: 'approved', rejectionNotes: '[]' }).where(eq(user.id, userId))
    } else {
      const existingNotes = (existing as unknown as Record<string, unknown>).rejectionNotes as string || '[]'
      ...
      await this.db.update(user).set({ status: 'rejected', rejectionNotes: JSON.stringify(notes) }).where(eq(user.id, userId))
    }
  ...
  async addUserNote(userId: string, dto: AddNoteDto) {
    ...
    await this.db.update(user).set({ rejectionNotes: JSON.stringify(notes) }).where(eq(user.id, userId))
