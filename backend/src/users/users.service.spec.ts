...
describe('UsersService', () => {
  ...
  describe('getProfile', () => {
    ...
    it('should return user profile', async () => {
      mockWhere.mockReturnValue({ limit: jest.fn().mockResolvedValue([{ id: 'u1', email: 'test@test.com', name: 'Test', role: 'karyawan', status: 'approved', emailVerified: false }]) });
  ...
  describe('updateProfile', () => {
    it('should reject update for other user', async () => {
      await expect(service.updateProfile('user-2', { nama: 'Test' }, 'user-1')).rejects.toThrow(ForbiddenException);
    });

    it('should reject duplicate email', async () => {
      mockWhere
        .mockReturnValueOnce({ limit: jest.fn().mockResolvedValue([{ id: 'user-1', status: 'approved' }]) })
        .mockReturnValueOnce({ limit: jest.fn().mockResolvedValue([{ id: 'user-2' }]) })
        .mockReturnValue({ limit: jest.fn().mockResolvedValue([]) });

      await expect(service.updateProfile('user-1', { email: 'existing@test.com' }, 'user-1')).rejects.toThrow(ConflictException);
    });

    it('should allow update with own email', async () => {
      mockWhere
        .mockReturnValueOnce({ limit: jest.fn().mockResolvedValue([{ id: 'user-1', status: 'approved' }]) })
        .mockReturnValueOnce({ limit: jest.fn().mockResolvedValue([{ id: 'user-1' }]) })
        .mockReturnValue({ limit: jest.fn().mockResolvedValue([{ id: 'user-1', email: 'same@test.com', name: 'test' }]) });

      const result = await service.updateProfile('user-1', { email: 'same@test.com' }, 'user-1');
      expect(result).toHaveProperty('id', 'user-1');
    });

    it('should reset status to pending if rejected', async () => {
      mockWhere
        .mockReturnValueOnce({ limit: jest.fn().mockResolvedValue([{ id: 'user-1', status: 'rejected' }]) })
        .mockReturnValue({ limit: jest.fn().mockResolvedValue([{ id: 'user-1' }]) });
  ...
  describe('updateUserStatus', () => {
    it('should approve user and clear notes', async () => {
      mockWhere.mockReturnValue({ limit: jest.fn().mockResolvedValue([{ id: 'u1', rejectionNotes: '[]' }]) });
    ...
    it('should reject with note', async () => {
      mockWhere.mockReturnValue({ limit: jest.fn().mockResolvedValue([{ id: 'u1', rejectionNotes: '[]' }]) });
      await service.updateUserStatus('u1', { status: 'rejected', note: 'Data tidak lengkap' });
