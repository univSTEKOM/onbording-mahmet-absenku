...
function recordAttempt(key: string, maxAttempts: number) {
  ...
  if (!r) { r = { count: 0 }; attempts.set(key, r) }
  r.count++
  if (r.count >= maxAttempts) {
    r.blockedAt = Date.now()
    r.duration = Math.min(BASE_BLOCK + (r.count - maxAttempts) * 15000, MAX_BLOCK)
  }
}
...
function loginRateLimiter(req: Request, res: Response, next: NextFunction) {
  ...
  const remaining = Math.max(isBlocked(emailKey, MAX_LOGIN_ATTEMPTS), isBlocked(ipKey, MAX_LOGIN_ATTEMPTS))
  if (remaining > 0) {
    res.status(429).json({
      success: false,
      error: { code: 'RATE_LIMIT', message: `Terlalu banyak percobaan. Coba lagi ${Math.ceil(remaining / 1000)} detik lagi.` },
    })
    return
  }
  ...
  res.json = function (data: Record<string, unknown>) {
    if (data?.token || data?.user) { clearAttempts(emailKey, ipKey) }
    else { recordAttempt(emailKey, MAX_LOGIN_ATTEMPTS); recordAttempt(ipKey, MAX_LOGIN_ATTEMPTS) }
...
function registerRateLimiter(req: Request, res: Response, next: NextFunction) {
  ...
  if (r && r.count >= MAX_REGISTER_ATTEMPTS && (now - (r.blockedAt || now)) < REGISTER_WINDOW) {
    res.status(429).json({
      success: false,
      error: { code: 'RATE_LIMIT', message: 'Terlalu banyak percobaan pendaftaran. Coba lagi dalam 1 menit.' },
    })
    return
  }
