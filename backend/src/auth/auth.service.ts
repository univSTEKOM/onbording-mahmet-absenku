...
export class AuthService {
  ...
  async register(dto: RegisterDto, req: Request) {
    ...
    if (!isAdminAction) {
      if (dto.password.length < 8) throw new BadRequestException('Password minimal 8 karakter')
      if (!/[A-Z]/.test(dto.password)) throw new BadRequestException('Password harus mengandung huruf kapital')
      if (!/[a-z]/.test(dto.password)) throw new BadRequestException('Password harus mengandung huruf kecil')
      if (!/[0-9]/.test(dto.password)) throw new BadRequestException('Password harus mengandung angka')
    }
