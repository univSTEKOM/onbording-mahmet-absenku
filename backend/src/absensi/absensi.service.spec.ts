...
describe('AbsensiService', () => {
  ...
  beforeEach(async () => {
    ...
    const module = await Test.createTestingModule({
      providers: [AbsensiService, { provide: DRIZZLE_DB, useFactory: db }],
    }).compile();
  ...
  describe('checkIn', () => {
    it('should reject non-admin check-in for other user', async () => {
      await expect(
        service.checkIn(
          { userId: 'user-2', tanggal: '2026-07-30' },
          'user-1',
          'karyawan',
        ),
    ...
    it('should reject check-in for non-existent user', async () => {
      ...
      await expect(
        service.checkIn(
          { userId: 'unknown', tanggal: '2026-07-30' },
          'unknown',
          'admin',
        ),
    ...
    it('should reject duplicate check-in', async () => {
      ...
      await expect(
        service.checkIn(
          { userId: 'user-1', tanggal: '2026-07-30' },
          'user-1',
          'admin',
        ),
  ...
  describe('checkOut', () => {
    ...
    it('should allow admin check-out for any user', async () => {
      ...
      mockReturning.mockResolvedValue([
        { id: 1, status: 'hadir', checkOut: new Date().toISOString() },
      ]);
