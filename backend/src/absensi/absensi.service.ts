...
export class AbsensiService {
  ...
  async checkIn(
    ...
    const [duplicate] = await this.db
      ...
      .from(absensi)
      .where(
        and(eq(absensi.userId, dto.userId), eq(absensi.tanggal, dto.tanggal)),
      )
      .limit(1);
  ...
  async checkOut(
    ...
    if (currentUserRole !== 'admin' && existing.userId !== currentUserId) {
      throw new ForbiddenException(
        'Anda hanya bisa check-out untuk diri sendiri',
      );
    }

    const checkOutResult = evaluateCheckOut();
    const currentPhotos =
      (existing.photos as Array<Record<string, string>>) || [];
    const photos = dto.photos?.length
    ...
    const [updated] = await this.db
      ...
      .set({
        ...
        subCategory: checkOutResult.pulangCepat
          ? checkOutResult.subCategory
  ...
  async list(params: AbsensiQueryParams) {
    ...
    if (params.tanggal_gte)
      filters.push(gte(absensi.tanggal, params.tanggal_gte));
    if (params.tanggal_lte)
      filters.push(lte(absensi.tanggal, params.tanggal_lte));
    ...
    const sortField =
      params._sort && allowedSortFields.includes(params._sort)
        ? params._sort
        : 'tanggal';
