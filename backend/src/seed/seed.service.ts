...
export class SeedService {
  ...
  async seed(password: string) {
    ...
    for (const p of PERSONAS) {
      const [existing] = await this.db.select({ id: user.id }).from(user).where(eq(user.email, p.email)).limit(1)
      if (existing) {
      ...
      try {
        ...
        if (response.status !== 200) {
          ...
          this.logger.warn(`Failed to create ${p.email}: ${errBody?.message || 'unknown error'}`)
          continue
        }
    ...
    const approvedUsers = created.filter((c) => c.persona.status === 'approved')
  ...
  private async generateAbsensi(users: { persona: typeof PERSONAS[number]; id: string }[]) {
    const today = new Date()
    const todayStr = today.toISOString().split('T')[0]
    const workDays = eachDay(APP_RELEASE, todayStr).filter((tgl) => !isWeekend(tgl))

    const records: Array<{
      userId: string; tanggal: string; checkIn: Date; checkOut: Date | null
      status: string; mainCategory: string; subCategory: string
      faceVerified: boolean; photos: never[]; keterangan: string
    }> = []
    ...
    for (const u of users) {
      ...
      for (const tgl of workDays) {
        const status = pickWeighted(
          Object.entries(u.persona.weights).map(([k, w]) => ({ weight: w, value: k })),
        )
        ...
        const checkIn = new Date(`${tgl}T${String(checkInHour).padStart(2, '0')}:${String(checkInMin).padStart(2, '0')}:${String(rand(0, 59)).padStart(2, '0')}.000Z`)

        const isPulangCepat = status === 'pulang_cepat'
        const checkOutHour = isPulangCepat ? rand(13, 15) : rand(16, 17)
        const checkOut = new Date(`${tgl}T${String(checkOutHour).padStart(2, '0')}:${String(rand(10, 50)).padStart(2, '0')}:${String(rand(0, 59)).padStart(2, '0')}.000Z`)
  ...
  private async generatePengajuan(users: { persona: typeof PERSONAS[number]; id: string }[]) {
    let inserted = 0
    const now = new Date()

    for (const s of SAMPLE_PENGAJUAN) {
      const match = users.find((u) => u.persona.email.startsWith(s.emailPrefix))
      if (!match) continue
